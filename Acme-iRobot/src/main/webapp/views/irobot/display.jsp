...
<br>
	...
	<jstl:if test="${iRobot.isDecommissioned eq true}">
		<spring:message var="status" code='irobot.isDecommissioned.true' />
	</jstl:if>
	<jstl:if test="${iRobot.isDecommissioned eq false}">
		<spring:message var="status" code='irobot.isDecommissioned.false' />
	</jstl:if>
	
	<jstl:if test="${isPrincipal }">
		<div>
			<strong><spring:message code="irobot.isDecommissioned" />: </strong>
...
<br>
<input type="button" name="back" value="<spring:message code="mp.back" />" onclick="javascript: relativeRedir('/iRobot/list.do')" />&nbsp;
